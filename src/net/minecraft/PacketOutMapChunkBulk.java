...
public class PacketOutMapChunkBulk implements Packet<PlayClientboundPacketListener> {
	...
	public PacketOutMapChunkBulk(List<Chunk> chunkList) {
		...
		for (int i = 0; i < chunkNumber; ++i) {
			...
			ChunkPacketData var5 = PacketOutChunkData.getChunkData(chunk, true, this.hasLight, '\uffff');
	...
	public void readData(PacketDataSerializer serializer) {
		...
		for (int i = 0; i < chunkNumber; ++i) {
			...
			this.packetData[i].blockData = new byte[PacketOutChunkData.calculateArraySize(Integer.bitCount(this.packetData[i].bitMap), this.hasLight, true)];
		}
