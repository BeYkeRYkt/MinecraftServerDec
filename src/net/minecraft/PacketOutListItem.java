...
public class PacketOutListItem implements Packet<PlayClientboundPacketListener> {

	private ListItemAction action;
	private final List<ListEntry> entries = Lists.newArrayList();

	public PacketOutListItem() {
	}

	public PacketOutListItem(ListItemAction action, EntityPlayer... players) {
		this.action = action;
	...
	public PacketOutListItem(ListItemAction action, Iterable<EntityPlayer> players) {
		this.action = action;
		for (EntityPlayer player : players) {
			entries.add(new ListEntry(player.getGameProfile(), player.ping, player.playerInteractManager.getGameMode(), player.getPlayerListName()));
		}
