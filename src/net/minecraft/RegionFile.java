...
public class RegionFile {
	...
	public RegionFile(File var1) {
		try {
			...
			if ((this.raf.length() & 4095L) != 0L) {
				for (var2 = 0; (long) var2 < (this.raf.length() & 4095L); ++var2) {
					this.raf.write(0);
				}
			}

			var2 = (int) this.raf.length() / 4096;
			...
			try {
				...
				if (var3 == 0) {
					...
					if (var4 + var5 > this.f.size()) {
						...
						this.raf.seek((long) (var4 * 4096));
						int var6 = this.raf.readInt();
						if (var6 > 4096 * var5) {
							return null;
						} else if (var6 <= 0) {
							return null;
						} else {
							byte var7 = this.raf.readByte();
							byte[] var8;
							if (var7 == 1) {
	...
	protected synchronized void a(int var1, int var2, byte[] var3, int var4) {
		try {
			...
			this.b(var1, var2, (int) (MinecraftServer.getCurrentMillis() / 1000L));
	...
	private void a(int var1, byte[] var2, int var3) throws IOException {
		this.raf.seek((long) (var1 * 4096));
		this.raf.writeInt(var3 + 1);
		this.raf.writeByte(2);
		this.raf.write(var2, 0, var3);
	}
	...
	private void a(int var1, int var2, int var3) throws IOException {
		...
		this.raf.seek((long) ((var1 + var2 * 32) * 4));
		this.raf.writeInt(var3);
	}

	private void b(int var1, int var2, int var3) throws IOException {
		this.e[var1 + var2 * 32] = var3;
		this.raf.seek((long) (4096 + (var1 + var2 * 32) * 4));
		this.raf.writeInt(var3);
	}
