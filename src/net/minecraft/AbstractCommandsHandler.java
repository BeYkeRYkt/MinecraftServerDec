...
public class AbstractCommandsHandler implements ICommandHandler {

	private static final Logger a = LogManager.getLogger();
	private final Map<String, ICommand> b = Maps.newHashMap();
	private final Set c = Sets.newHashSet();

	public int handleCommand(CommandSenderInterface var1, String var2) {
		var2 = var2.trim();
		if (var2.startsWith("/")) {
		...
		if (var5 == null) {
			...
			if (var6 > -1) {
				...
				while (var10.hasNext()) {
					...
					if (this.handleCommand(var1, var3, var5, var2)) {
						++var7;
					}
				}
	...
	protected boolean handleCommand(CommandSenderInterface sender, String[] args, ICommand command, String commandLabel) {
		ChatMessage var6;
		try {
			...
			var6.getChatModifier().setColor(EnumChatFormat.RED);
			sender.sendChatMessage(var6);
			a.error("Couldn\'t process command: \'" + commandLabel + "\'", var9);
		}
	...
	public ICommand a(ICommand var1) {
		this.b.put(var1.getName(), var1);
		this.c.add(var1);
		Iterator var2 = var1.getAliases().iterator();
	...
	public List a(CommandSenderInterface var1, String var2, Position var3) {
		...
		if (var4.length == 1) {
			...
			while (var7.hasNext()) {
				...
				if (AbstractCommand.startWith(var5, (String) var8.getKey()) && ((ICommand) var8.getValue()).a(var1)) {
	...
	public List a(CommandSenderInterface var1) {
